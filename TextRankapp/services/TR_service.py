...
sent = ['안녕하세요 요즘 뭐해?<br> 나 개발 공부해<br> 요즘 개발 공부하지<br><br> 너도 한다고 들었는데<br> 장고 채널스 공부하고있는데<br><br>']


# scan_vocabulary()는 min_count 이상 등장한 단어를 추출하고, string으로 들어온 문장을 쪼개 list로 만들고 Counter 클래스를 이용해 단어의 갯수(빈도수)를 센다.
...
# 두 단어의 co-occurrence(동시 출현 횟수)를 계산하는데, 두 단어 간격이 window=2인 횟수를 말한다. 그리고 결과를 dict_to_mat 함수로 sparse matrix로 변환한다.
...
# 단어를 key로 빈도수를 value로 한 딕셔너리를 생성하고 value(빈도수)기준으로 정렬한 뒤 단어에 인덱스를 부여한다.
...
# PageRank를 계산하는 함수
# 컬럼의 합이 1이되도록 L1 정규화를 하고 이를 A라 하며 A*R은 Rj(column j에서 row i로의 랭킹)의 전달되는 값을 의미한다. 이 값에 df를 곱하고 모든 마디에 1 - df를 더하고 이를 max_iter만큼 반복
